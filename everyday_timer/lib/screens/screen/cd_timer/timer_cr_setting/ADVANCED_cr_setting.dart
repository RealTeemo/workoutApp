...
class AdvancedSettingState extends ConsumerState<AdvancedSetting> {
  ...
  void submitNewTimerData() {
    ...
    temp = CdTimer(
        time: enteredTotalTime,
        title: presetTitleController.text,
        sets: selectedSets);
    temp2 = StoredTimer(time: enteredTotalTime, title: titleController.text);
    temp.addTimer(temp2);
    if (breakSecs != 0) {
      temp2 = StoredTimer(time: breakSecs, title: "Break");
      temp.addTimer(temp2);
    }
      ...
      for (int i = 0; i < generatedButtons.length; i++) {
        ...
        if (enteredtotalRestTime != 0) {
          temp2 = StoredTimer(
              ...
              title: generatedButtons[i].titleController.text);
          temp.addTimer(temp2);
        }
