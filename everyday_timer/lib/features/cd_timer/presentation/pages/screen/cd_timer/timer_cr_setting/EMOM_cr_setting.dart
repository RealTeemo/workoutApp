...
class _EmomSettingState extends ConsumerState<EmomSetting> {
  ...
  void submitNewTimerData() {
    ...
    for (int i = 0; i < forVal; i++) {
      temp2 = StoredTimer(time: enteredTotalTime, title: "Work${i + 1}", type: WorkType.work.toString());
      sendTimer.addTimer(temp2);
    }

    if (generatedButtons.isNotEmpty) {
      final enteredtotalRestTime = generatedButtons[0].selectedMins * 60 +
          generatedButtons[0].selectedSecs;
                ...
                Row(
                  ...
                  children: [
                    ...
                    DropdownIntSelector(
                      ...
                      onChanged: (newValue) {
                        setState(() {
                          forVal = newValue!; // Update the selected value
                          print(breakMins);
                          print(breakSecs);
                        });
                      },
